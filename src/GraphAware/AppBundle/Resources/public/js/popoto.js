...
popoto = function () {
    ...
    popoto.taxonomy.updateCount = function (taxonomyData) {
        var query = popoto.query.generateTaxonomyCountQuery(taxonomyData.label);
        popoto.logger.info("Label count query ==> " + query);
        ...
        (function (mainData) {
            popoto.rest.post(
                ...
                .done(function (returnedData) {
                    var count = returnedData.data[0][0];
                    var id = mainData.id;
                    d3.select("#" + id)
                        .select(".ppt-count")
                        .text(" (" + count + ")");
    ...
    popoto.graph.createGraphArea = function () {
        ...
        popoto.graph.svg = htmlContainer
            .append("svg")
    ...
    popoto.graph.node.updateCount = function () {
        // The count is updated for each node except for GROUP as they don't need count.
        popoto.graph.force.nodes()
            .filter(function (d) {
                ...
                popoto.logger.info("Count (" + node.label + ") ==> " + query);
                ...
                popoto.rest.post(
                    ...
                    .done(function (returnedData) {
                        node.count = returnedData.data[0][0];
    ...
    popoto.graph.node.addNewElements = function () {
        ...
        }).on("contextmenu", popoto.graph.node.rightClickNode);
     ...
     * Depending on the node type it will generate ellipse with text, image or svg paths.
     *
     * @param gNewNodeElements
     */
    popoto.graph.node.addMiddlegroundElements = function (gNewNodeElements) {
        ...
        //----------------
        // Image nodes
        var middleImages = middle.filter(function (d) {
            return popoto.provider.getNodeDisplayType(d) === popoto.provider.NodeDisplayTypes.IMAGE;
        });
        middleImages.append("image")
            .attr("class", "ppt-node-image");

        //----------------
        // SVG nodes
        var middleSVG = middle.filter(function (d) {
        ...
        //----------------
        // Text nodes
        var ellipseNodes = middle.filter(function (d) {
            var nodeDisplayType = popoto.provider.getNodeDisplayType(d);
            return nodeDisplayType === popoto.provider.NodeDisplayTypes.TEXT;
        });
        ellipseNodes.append("ellipse")
            .attr("rx", popoto.graph.node.ELLIPSE_RX)
            .attr("ry", popoto.graph.node.ELLIPSE_RY);

        var textNodes = middle.filter(function (d) {
            return popoto.provider.isTextDisplayed(d);
        });
        textNodes.append('text')
            .attr('x', 0)
            .attr('y', popoto.graph.node.TEXT_Y)
            // To center text, "alignment-baseline: middle" should be used instead of attr y = 8 but is not supported by IE and Firefox.
    ...
    popoto.graph.node.updateMiddlegroundElements = function () {
        ...
        //-------------------------------
        // Update IMAGE nodes
        popoto.graph.node.svgNodeElements.selectAll(".ppt-g-node-middleground").filter(function (d) {
            return popoto.provider.getNodeDisplayType(d) === popoto.provider.NodeDisplayTypes.IMAGE;
        }).selectAll(".ppt-node-image")
        ...
        //-------------------------
        // Update TEXT nodes
        var ellipseMiddle = popoto.graph.node.svgNodeElements.selectAll(".ppt-g-node-middleground").filter(function (d) {
            var nodeDisplayType = popoto.provider.getNodeDisplayType(d);
            return nodeDisplayType === popoto.provider.NodeDisplayTypes.TEXT;
        });

        // Set class according to node type
        ellipseMiddle.selectAll("ellipse")
            .attr("rx", popoto.graph.node.ELLIPSE_RX)
            .attr("ry", popoto.graph.node.ELLIPSE_RY)
            .attr("class", function (d) {
        ...
        //-------------------------
        // Update SVG nodes
        var svgMiddle = popoto.graph.node.svgNodeElements.selectAll(".ppt-g-node-middleground").filter(function (d) {
            return popoto.provider.getNodeDisplayType(d) === popoto.provider.NodeDisplayTypes.SVG;
        })
            // Add D3.js nested data with all paths required to render the svg element.
            .select("g").selectAll("path").data(function (d) {
        ...
        popoto.graph.node.svgNodeElements.selectAll(".ppt-g-node-middleground")
            .selectAll("path")
            .attr("d", function (d) {
                return d.d;
            })
        ...
        // Update text
        var textMiddle = popoto.graph.node.svgNodeElements.selectAll(".ppt-g-node-middleground").filter(function (d) {
            return popoto.provider.isTextDisplayed(d);
        });
        textMiddle.selectAll("text")
            .attr('y', popoto.graph.node.TEXT_Y)
            .attr("class", function (d) {
                switch (d.type) {
    ...
    popoto.graph.node.chooseNodeClick = function (clickedNode) {
        ...
        if (!popoto.graph.node.chooseWaiting && !clickedNode.immutable) {
            ...
            // Get or update data and expand node.
            var query = popoto.query.generateValueQuery(clickedNode);
            popoto.logger.info("Values (" + clickedNode.label + ") ==> " + query);
    ...
    popoto.graph.node.parseResultData = function (data) {
        ...
        for (var x = 0; x < data.data.length; x++) {
            ...
            for (var i = 0; i < data.columns.length; i++) {
                obj[data.columns[i]] = data.data[x][i];
            }
     ...
     *
     */
    popoto.graph.node.rightClickNode = function () {
        ...
        if (!clickedNode.linkExpanded && !popoto.graph.node.linkWaiting) {
            popoto.graph.node.linkWaiting = true;

            var query = popoto.query.generateLinkQuery(clickedNode);
            popoto.logger.info("Relations (" + clickedNode.label + ") ==> " + query);
            ...
            popoto.rest.post(
                ...
                .done(function (data) {
                    ...
                    if (parsedData.length <= 0) {
                        ...
                        // As no relation have been found the standard remove value action can be executed.
                        if (clickedNode.value != null && !clickedNode.valueExpanded) {
        ...
        } else if (clickedNode.value != null && !clickedNode.valueExpanded && !clickedNode.immutable) {
...
// QUERY ------------------------------------------------------------------------------------------------------------
    ...
    popoto.query.generateTaxonomyCountQuery = function (label) {
        ...
        if (constraintAttr === popoto.query.NEO4J_INTERNAL_ID) {
            return "match (n:`" + label + "`) return count(DISTINCT ID(n))"
        } else {
            return "match (n:`" + label + "`) return count(DISTINCT n." + constraintAttr + ")"
        }
    };
    ...
    popoto.query.generateQueryElements = function (rootNode, selectedNode, links, isConstraintNeeded) {
        ...
        if (rootNode.value && !(rootNode === selectedNode) && (isConstraintNeeded || rootNode.immutable)) {
        ...
        links.forEach(function (l) {
            ...
            if (targetNode.value && !(targetNode === selectedNode) && isConstraintNeeded) {
                ...
                if (constraintAttr === popoto.query.NEO4J_INTERNAL_ID) {
                    matchElements.push("(" + sourceNode.internalLabel + ":`" + sourceNode.label + "`)-[:`" + l.label + "`]->(" + targetNode.internalLabel + ":`" + targetNode.label + "`)");
                    whereElements.push("ID(" + targetNode.internalLabel + ") = " + targetNode.value.internalID);
                } else {
                    if (typeof constraintValue === "boolean" || typeof constraintValue === "number") {
                        matchElements.push("(" + sourceNode.internalLabel + ":`" + sourceNode.label + "`)-[:`" + l.label + "`]->(" + targetNode.internalLabel + ":`" + targetNode.label + "`{`" + constraintAttr + "`:" + constraintValue + "})");
                    } else {
                        matchElements.push("(" + sourceNode.internalLabel + ":`" + sourceNode.label + "`)-[:`" + l.label + "`]->(" + targetNode.internalLabel + ":`" + targetNode.label + "`{`" + constraintAttr + "`:\"" + constraintValue + "\"})");
                    }
                }
    ...
    popoto.query.getRelevantLinks = function (rootNode, targetNode, initialLinks) {
        ...
        // Filter all links to keep only those containing a value or beeing the selected node.
        links.forEach(function (l) {
    ...
    popoto.query.generateLinkQuery = function (targetNode) {
        ...
        var matchElements = popoto.query.generateQueryElements(popoto.graph.getRootNode(), targetNode, linksToRoot, false).matchElements,
            returnElements = [],
            endElements = [];

        matchElements.push("(" + targetNode.internalLabel + ":`" + targetNode.label + "`)-[r]->(x)");
        ...
        return "MATCH " + matchElements.join(", ") + " RETURN " + returnElements.join(", ") + " " + endElements.join(" ");
    };
    ...
    popoto.query.generateNodeCountCypherQuery = function (countedNode) {
        ...
        if (countAttr === popoto.query.NEO4J_INTERNAL_ID) {
            returnElements.push("count(DISTINCT ID(" + countedNode.internalLabel + "))");
        } else {
            returnElements.push("count(DISTINCT " + countedNode.internalLabel + "." + countAttr + ")");
        }
    ...
    popoto.result.parseResultData = function (data) {
        ...
        for (var x = 0; x < data.data.length; x++) {
            ...
            for (var i = 0; i < data.columns.length; i++) {
                // Some results can be a array as collect is used in query
                // So all values are converted to string
                obj.attributes[data.columns[i]] = "" + data.data[x][i];
            }
    ...
    popoto.result.updateResults = function () {
        if (popoto.result.hasChanged) {
            ...
            popoto.logger.info("Results ==> " + query);
            ...
            if (popoto.result.resultCountListeners.length > 0) {
                var queryCount = popoto.query.generateResultCypherQueryCount();
                popoto.logger.info("Count Results ==> " + queryCount);
                popoto.rest.post(
                    {
                        "query": queryCount,
                        "params": {}
                    ...
                    .done(function (data) {
                        var count = data.data[data.columns.indexOf("count")];
    ...
    popoto.provider.DEFAULT_PROVIDER = Object.freeze(
        {
             ...
             *
             * The default behavior is to generate an &lt;H3&gt; element containing the constraint attribute value of the result.
             *
            ...
            "displayResults": function (pElmt) {
                ...
                pElmt.append("h3")
                    .text(function (result) {
                        var constraintAttribute = popoto.provider.getConstraintAttribute(result.label);
                        if (constraintAttribute === popoto.query.NEO4J_INTERNAL_ID) {
                            return result.attributes[popoto.query.NEO4J_INTERNAL_ID.queryInternalName];
                        } else {
                            return result.attributes[constraintAttribute];
                        }
                    });
            }
